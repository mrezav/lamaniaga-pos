...
import { Store, User, PlusCircle, ArrowRight, Loader2 } from "lucide-react"
...
export function StoreSelector({ stores, userId }: StoreSelectorProps) {
  ...
  const handleSelectStore = async (store: StoreData) => {
    ...
    try {
      ...
      showToast(err.message || "Gagal masuk ke toko", "error")
      setSelectedId(null)
    }
  }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-12 px-4 space-y-10 animate-in fade-in duration-500">
      <div className="text-center space-y-3">
        <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">Pilih Toko untuk Mengelola</h1>
        <p className="text-slate-500 max-w-lg mx-auto text-base font-medium leading-relaxed">
          Anda terdaftar di beberapa toko. Silakan pilih salah satu di bawah ini untuk mengakses dashboard POS.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        {stores.map((store) => {
          ...
          return (
            <div
              key={store.id}
              onClick={() => !isAnyLoading && handleSelectStore(store)}
      ...
      <div className="flex justify-center pt-6 border-t border-slate-200/60">
        <Link href="/store/create">
          <button className="h-13 px-6 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold transition-all shadow-md shadow-slate-900/10 hover:shadow-slate-900/20 flex items-center gap-2 text-sm">
