...
export function StoreLayoutClient({ children, store, profile }: StoreLayoutClientProps) {
  ...
  const handleLogout = async () => {
    try {
      ...
      showToast(err.message || "Gagal melakukan logout", "error")
    }
  }
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isSidebarCollapsed && (
              <div className="flex flex-col truncate">
                <span className="font-extrabold text-white text-sm tracking-tight leading-tight">{store.name}</span>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Pemilik Toko</span>
              </div>
            )}
          </div>
          {!isSidebarCollapsed && (
              ...
              {!isSidebarCollapsed && (
                <div className="flex items-center justify-between w-full">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Kode Toko</span>
                    <span className="text-xs font-mono font-bold mt-0.5">{store.joinCode}</span>
                  </div>
                  {isCopied ? <Check className="w-4 h-4 text-emerald-500 shrink-0" /> : <Copy className="w-4 h-4 text-slate-400 shrink-0 group-hover:text-white" />}
                </div>
              )}
            </div>
          )}
        ...
        <div className="p-4 border-t border-[#1E293B] shrink-0 text-center flex flex-col items-center justify-center">
          {!isSidebarCollapsed ? (
            <div className="space-y-1">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Trademark</p>
              <p className="text-xs text-slate-400 font-semibold font-mono">© 2026 Lamaniaga</p>
            </div>
          ) : (
            <span className="text-xs font-black font-mono text-slate-600">L'26</span>
          )}
        </div>
      </aside>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-3 w-56 bg-white border border-slate-100 rounded-2xl shadow-xl shadow-slate-100/60 z-40 py-2 overflow-hidden animate-in fade-in slide-in-from-top-3 duration-200">
                    <div className="px-4 py-3 border-b border-slate-50">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Nama Akun</p>
