...
import { getStoreMembersAction, updateMemberStatusAction } from "../services/join-actions"
...
export function StoreMembersTable({ storeId }: StoreMembersTableProps) {
  ...
  const mutation = useMutation({
    mutationFn: async ({ memberId, status }: { memberId: string; status: "active" | "rejected" }) => {
    ...
    onSuccess: (_, variables) => {
      const actionName = variables.status === "active" ? "menyetujui" : "menolak"
    ...
    onError: (err: any) => {
      showToast(err.message || "Gagal memperbarui status keanggotaan.", "error")
    },
  })

  if (isLoading) {
    return (
      <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-md flex flex-col items-center justify-center min-h-[300px] space-y-4">
        <Loader2 className="w-8 h-8 text-blue-600 animate-spin" />
        <span className="text-slate-400 text-sm font-semibold">Memuat daftar keanggotaan...</span>
      </div>
    )
  }
  ...
  if (error || !data) {
    return (
      <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-md flex flex-col items-center justify-center min-h-[300px] text-center space-y-4">
        <p className="text-red-500 font-bold">Gagal memuat data anggota.</p>
    ...
    <div className="bg-white rounded-3xl border border-slate-100 shadow-lg shadow-slate-100/40 overflow-hidden">
      ...
      <div className="p-6 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-extrabold text-slate-800 text-sm">Daftar Keanggotaan & Pelamar</h3>
            <p className="text-slate-400 text-xs font-semibold">Kelola siapa saja yang memiliki akses ke kasir Anda</p>
          </div>
        </div>
      ...
      {data.length === 0 ? (
        <div className="p-12 text-center flex flex-col items-center justify-center space-y-4">
          ...
          <div className="space-y-1">
            <p className="text-slate-700 font-extrabold text-sm">Belum Ada Anggota</p>
            <p className="text-slate-400 text-xs max-w-sm font-medium leading-relaxed">
              Bagikan kode akses di samping agar staf kasir Anda dapat melamar untuk bergabung dengan toko ini.
            </p>
          </div>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((member) => {
                ...
                const isMutationPending = mutation.isPending && mutation.variables?.memberId === member.id
                  ...
                  <tr key={member.id} className="hover:bg-slate-50/30 transition-colors group">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 border border-slate-200/50 flex items-center justify-center font-bold text-sm shrink-0 uppercase">
                          {member.avatarUrl ? (
                            <img src={member.avatarUrl} alt={member.fullName} className="w-full h-full object-cover rounded-xl" />
                          ) : (
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <span className="text-[10px] text-slate-400 font-mono mt-0.5 truncate">{member.userId}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-1.5">
                        <Award className={`w-3.5 h-3.5 ${member.role === 'owner' ? 'text-blue-500' : 'text-slate-400'}`} />
                        <span className="text-xs text-slate-600 font-semibold capitalize">
                          {member.role === "owner" ? "Pemilik" : member.role === "manager" ? "Manajer" : "Kasir"}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6 text-right">
                      {isPending ? (
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            onClick={() => mutation.mutate({ memberId: member.id, status: "active" })}
                          ...
                          >
                            {isMutationPending && mutation.variables?.status === "active" ? (
                          ...
                          <Button
                            onClick={() => mutation.mutate({ memberId: member.id, status: "rejected" })}
                            disabled={isMutationPending}
                          ...
                          >
                            {isMutationPending && mutation.variables?.status === "rejected" ? (
                              <Loader2 className="w-3 h-3 animate-spin" />
                            ) : (
                        ...
                        <span className="text-xs text-slate-400 font-semibold italic">Sudah Diproses</span>
                      )}
                    </td>
                  </tr>
                )
