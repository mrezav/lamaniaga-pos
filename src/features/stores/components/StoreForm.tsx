...
import { createStoreSchema, type CreateStoreSchemaType } from "../schemas/store-schema"
import { generateUniqueSlug, createStoreTransaction } from "../services/store-actions"
import { useToastStore } from "@/store/useToastStore"
import { useMutation } from "@tanstack/react-query"
import { useRouter } from "next/navigation"
import { createClient } from "@/lib/supabase/client"
import { useState, useEffect } from "react"
import { Loader2, Store, UploadCloud, MapPin, Phone, Image as ImageIcon, Sparkles } from "lucide-react"
...
export function StoreForm({ userId }: StoreFormProps) {
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      showToast(error.message || "Gagal membuat toko. Silakan coba kembali.", "error")
    },
  })
        ...
        {bannerPreview ? (
          <>
            ...
            <label className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer text-white font-semibold gap-2">
              ...
              Ganti Banner Toko
              <input type="file" accept="image/*" className="hidden" onChange={handleBannerChange} disabled={isLoading} />
            </label>
          </>
        ) : (
          <label className="flex flex-col items-center justify-center cursor-pointer w-full h-full p-6 text-slate-500">
            <UploadCloud className="w-8 h-8 text-slate-400 mb-2" />
            <span className="font-semibold text-sm">Unggah Banner Toko</span>
            <span className="text-xs text-slate-400 mt-1">Format gambar (Maks. 5MB)</span>
            <input type="file" accept="image/*" className="hidden" onChange={handleBannerChange} disabled={isLoading} />
          </label>
        )}
        {errors.banner && (
        ...
        <div className="space-y-6 flex flex-col items-center">
          <div className="relative w-40 h-40 rounded-full bg-white shadow-md border border-slate-100 flex items-center justify-center overflow-hidden group">
            {logoPreview ? (
              ...
              <label className="flex flex-col items-center justify-center cursor-pointer w-full h-full p-4 text-slate-400 hover:bg-slate-50/50 transition-colors">
                ...
                <span className="text-xs font-medium text-center">Pilih Logo</span>
                <input type="file" accept="image/*" className="hidden" onChange={handleLogoChange} disabled={isLoading} />
              </label>
            )}
            {errors.logo && (
          ...
          <div className="w-full bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-blue-500" /> Auto-Generated Slug
            </h3>
            <div className="flex items-center gap-2">
              ...
              />
              {isCheckingSlug && <Loader2 className="w-4 h-4 text-blue-500 animate-spin shrink-0" />}
            </div>
            {errors.slug && (
              <p className="text-xs text-red-600 font-medium leading-relaxed">{errors.slug.message}</p>
            )}
            <p className="text-[10px] text-slate-400">
              *Slug dihasilkan otomatis berdasarkan Nama Toko Anda dan divalidasi keunikannya.
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="space-y-2">
            ...
            <input
              ...
              className={`w-full px-4 py-3 border rounded-xl outline-none transition-all bg-slate-50/50 font-medium ${
                errors.name ? "border-red-300 focus:ring-2 focus:ring-red-200" : "border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              }`}
              placeholder="Contoh: Toko Sejahtera Sentosa"
            />
            {errors.name && (
              <p className="text-xs text-red-600 font-medium leading-relaxed">{errors.name.message}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              disabled={isLoading}
              className={`w-full px-4 py-3 border rounded-xl outline-none transition-all bg-slate-50/50 font-medium ${
                errors.phoneNumber ? "border-red-300 focus:ring-2 focus:ring-red-200" : "border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              }`}
              placeholder="Contoh: 08123456789"
            />
            {errors.phoneNumber && (
              <p className="text-xs text-red-600 font-medium leading-relaxed">{errors.phoneNumber.message}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className={`w-full px-4 py-3 border rounded-xl outline-none transition-all bg-slate-50/50 font-medium resize-none ${
                errors.address ? "border-red-300 focus:ring-2 focus:ring-red-200" : "border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              }`}
              placeholder="Tulis alamat toko Anda secara rinci di sini..."
            />
            {errors.address && (
              <p className="text-xs text-red-600 font-medium leading-relaxed">{errors.address.message}</p>
            )}
          </div>
