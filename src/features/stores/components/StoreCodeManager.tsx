...
export function StoreCodeManager({ storeId, initialCode }: StoreCodeManagerProps) {
  ...
  const handleCopy = async () => {
    if (!code) {
      showToast("Belum ada kode yang di-generate. Silakan buat kode terlebih dahulu.", "error")
      return
    }
    try {
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      showToast(error.message || "Gagal memperbarui kode toko.", "error")
    },
  })
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-lg shadow-slate-100/40 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-extrabold text-slate-800 text-sm">Kode Akses Toko</h3>
          <p className="text-slate-400 text-xs font-semibold">Bagikan kode ini ke staf Anda</p>
        </div>
      </div>

      {/* Code Display Box */}
      <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex items-center justify-between gap-4">
        <span className={`font-mono text-xl font-black tracking-widest text-slate-700 ${!code && "text-slate-300 font-sans text-sm tracking-normal font-semibold"}`}>
          {code || "Belum ada kode"}
        </span>
      ...
      >
        {mutation.isPending ? (
          ...
          <>
            ...
            <span>{code ? "Generate Ulang Kode" : "Generate Kode Baru"}</span>
          </>
        )}
      </Button>
    </div>
